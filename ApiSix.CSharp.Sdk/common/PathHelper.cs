...
namespace ApiSix.CSharp
{
    ...
    public static class PathHelper
    {
        ...
        static PathHelper()
        {
            ...
            // 环境变量
            if (dir.IsNullOrWhiteSpace()) dir = Runtime.GetEnvironmentVariable("BasePath");

            if (!dir.IsNullOrWhiteSpace()) BaseDirectory = dir;
            ...
            if (dir.IsNullOrWhiteSpace()) dir = Environment.CurrentDirectory;
            ...
            if (!dir.IsNullOrWhiteSpace()) BasePath = GetPath(dir, 1);
        }
        #endregion
        ...
        public static String CombinePath(this String path, params String[] ps)
        {
            ...
            {
                if (!item.IsNullOrWhiteSpace()) path = Path.Combine(path, item);
            }
            return path;
        }
        #endregion
        ...
        {
            if (destDir.IsNullOrWhiteSpace()) destDir = Path.GetDirectoryName(fi.FullName).CombinePath(fi.Name);
        ...
        {
            if (destFile.IsNullOrWhiteSpace()) destFile = fi.Name + ".zip";
        ...
        public static String[] CopyIfNewer(this DirectoryInfo di, String[] source, String exts = null, Boolean allSub = false)
        {
            ...
            foreach (var item in source)
            {
                ...
                XTrace.WriteLine("复制 {0} => {1}", item, cur);
        ...
        {
            if (destFile.IsNullOrWhiteSpace()) destFile = di.Name + ".zip";
