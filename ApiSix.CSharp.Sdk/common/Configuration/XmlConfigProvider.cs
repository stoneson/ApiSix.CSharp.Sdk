    ...
    public class XmlConfigProvider : FileConfigProvider
    {
        ...
        private void ReadNode(XmlReader reader, IConfigSection section)
        {
            while (true)
            {
                ...
                while (reader.NodeType is XmlNodeType.Comment || reader.NodeType is XmlNodeType.Whitespace) reader.Skip();
                ...
                // 遇到下一层节点
                if (reader.NodeType is XmlNodeType.Element || reader.NodeType is XmlNodeType.Comment)
                    ReadNode(reader, cfg);
