...
namespace ApiSix.CSharp
{
    ...
    public class AssemblyX
    {
        ...
        public String FileVersion
        {
            get
            {
                if (_FileVersion == null)
                {
                    ...
                    {
                        var p = ver.IndexOf('+');
                        if (p > 0) ver = ver.Substring(0, p);// ver[..p];
                    }
                    _FileVersion = ver;
                }
        ...
        private static Assembly OnAssemblyResolve(Object sender, ResolveEventArgs args)
        {
            ...
            //if (flag)
            XTrace.WriteLine("[{0}]请求加载[{1}]", args.RequestingAssembly?.FullName, args.Name);
            //if (!flag) return null;
            ...
            try
            {
                ...
                if (!file.IsNullOrEmpty())
                {
                    var name = args.Name;
                    var p = name.IndexOf(',');
                    if (p > 0) name = name.Substring(0, p); //name[..p];
        ...
        #region 插件
        private readonly ConcurrentDictionary<Type, List<Type>> _plugins = new ConcurrentDictionary<Type, List<Type>>();
        ...
        public static Type GetType(String typeName, Boolean isLoadAssembly)
        {
            ...
            {
                var elemType = GetType(typeName.Substring(0, typeName.Length - 2), isLoadAssembly); //GetType(typeName[0..^2], isLoadAssembly);
        ...
        public static ICollection<String> AssemblyPaths
        {
            get
            {
                if (_AssemblyPaths == null)
                {
                    ...
                    {
                        var plugin = PluginPath.GetFullPath();
                        if (!set.Contains(plugin)) set.Add(plugin);

                        plugin = PluginPath.GetBasePath();
                        if (!set.Contains(plugin)) set.Add(plugin);
                    }
        ...
        private static Assembly OnResolve(String name)
        {
            ...
            if (p > 0)
            {
                name = name.Substring(0, p); //name[..p];
                foreach (var item in AppDomain.CurrentDomain.GetAssemblies())
                {
        ...
        public static DateTime GetCompileTime(String version)
        {
            ...
            if (d <= y && d >= y - 10)
            {
                ...
                else
                {
                    ...
                    if (p > 0)
                    {
                        s = str.Substring(0, p).ToInt(); //str[..p].ToInt();
                        if (s > 0)
                        {
                            if (s >= 200) dt = dt.AddMonths(s / 100 - 1);
                            s %= 100;
                        ...
                        if (str.Length >= 4 + 1 + 4)
                        {
                            s = str.Substring(str.Length - 4).ToInt(); //str[^4..].ToInt();
