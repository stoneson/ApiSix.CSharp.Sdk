...
namespace ApiSix.CSharp
{
    ...
    public static class Utility
    {
        ...
        /// <param name="format">格式字符串，默认s格式化到秒，ms格式化到毫秒</param>
        /// <returns></returns>
        public static DateTimeOffset Trim(this DateTimeOffset value, String format = "s") => new DateTimeOffset(Convert.Trim(value.DateTime, format), value.Offset);
    ...
    public class DefaultConvert
    {
        ...
        public virtual DateTime ToDateTime(Object value, DateTime defaultValue)
        {
                ...
                (value is IList<String> list && list.Count > 0 && (str = list[0]) != null))
            {
                ...
                {
                    utc = true;
                    str = str.Substring(0, str.Length - 4); //str = str[0..^4];
                }
